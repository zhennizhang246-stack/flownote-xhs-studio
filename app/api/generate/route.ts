...
type GeneratedDraft = { title:string; titleOptions?:string[]; coverTitle:string; coverSubtitle:string; body:string; tags:string[]; highlights:string[]; riskNotes:string[]; coverIndex:number; mode:string };
...
export async function POST(request: Request) {
  try {
    ...
    const content:Array<Record<string,unknown>>=[{type:"input_text",text:"你是个人室内设计工作室的高级内容秘书。必须依据上传的项目实景图生成小红书封面与原创文案。先识别空间类型、材质、灯光、动线、功能和画面情绪，再选择最适合封面的图片序号。只借鉴高互动设计项目笔记的结构，不复制原句，不虚构地点、面积、客户身份、材料品牌或完工事实。生成 3 个差异明确、可选择的小红书笔记标题，分别侧重空间情绪、设计亮点、居住价值；每个标题不超过 20 个汉字，避免标题党。titleOptions 必须正好包含 3 个标题，title 必须等于 titleOptions 的第一项。只返回 JSON，字段为 title, titleOptions, coverTitle, coverSubtitle, body, tags, highlights, riskNotes, coverIndex。项目事实："+JSON.stringify(project)}];
