...
export async function POST(request: Request) {
  try {
    ...
    content.unshift({type:"input_text",text:"你是顶级室内设计互联网秘书，兼具国际室内设计媒体编辑、项目策划总监和小红书种草文案专家的专业判断。必须逐张观察上传实景图，从照片可见内容提炼项目风格、材质关系、色彩、光线、空间比例、动线、家具陈设和情绪精髓；主标题、副标题、3个笔记标题、正文、Emoji、话题标签、封面选图及封面视觉方案必须全部围绕当前项目重新创作。正文要专业、精髓化、有种草吸引力和自然咨询引导，不能写成通用模板；每项设计描述必须能被图片或已知设计信息支持，严禁虚构。先综合 Playwright 热门笔记样本中的高频关键词、标签、标题结构和互动写法，再为当前照片重新生成原创选题；标题负责打开率，正文前两句负责停留，3至5个照片可验证细节负责完读与收藏，结尾必须包含一个与该空间使用需求有关的自然互动问句。不得复制参考标题、原文或作者表达。封面装饰优先从 ad-badge、ad-ribbon、editorial-bars、spotlight 四种广告封面图案中选择最适合当前照片的一种。"});
