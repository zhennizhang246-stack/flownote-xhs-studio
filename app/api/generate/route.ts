...
type GeneratedDraft = { title:string; titleOptions?:string[]; coverTitle:string; coverSubtitle:string; coverStyle?:Record<string,unknown>; body:string; tags:string[]; highlights:string[]; riskNotes:string[]; coverIndex:number; mode:string };
...
export async function POST(request: Request) {
  try {
    ...
    const content:Array<Record<string,unknown>>=[{type:"input_text",text:"你是个人室内设计工作室的高级内容秘书。必须依据上传的项目实景图生成小红书封面与原创文案。先识别空间类型、材质、灯光、动线、功能和画面情绪，再选择最适合封面的图片序号。必须针对当前空间类型改变叙事重点、标题角度、封面选图和视觉样式，不能把住宅客厅、卧室、办公、酒店、商业和展厅写成同一套模板。当前空间专属策略："+spaceDesignGuidance(project.projectType,project.category)+"必须使用最近3篇流量研究中已提炼的文案结构、封面层级、受众洞察和可复用方法进行升级，但不得复制标题、原句、段落或封面版式，不得让参考内容覆盖项目图片与真实事实。只借鉴抽象规律，不虚构地点、面积、客户身份、材料品牌或完工事实。生成 3 个差异明确、可选择的小红书笔记标题，分别侧重空间情绪、设计亮点、实际使用价值；每个标题不超过 20 个汉字，避免标题党。正文必须至少写出两项与当前空间类型直接相关、且能从图片或项目资料验证的设计细节。titleOptions 必须正好包含 3 个标题，title 必须等于 titleOptions 的第一项。根据项目图片、空间策略与研究规律推荐 coverStyle：fontFamily 只能是 serif/sans/kai；titleColor、subtitleColor、overlayColor、patternColor 使用6位十六进制颜色；overlayOpacity 为0-90；pattern 只能是 none/frame/grid/dots/corners；titleSize 为52-120；align 只能是 left/center；position 只能是 top/middle/bottom。只返回 JSON，字段为 title, titleOptions, coverTitle, coverSubtitle, coverStyle, body, tags, highlights, riskNotes, coverIndex。项目事实："+JSON.stringify(project)+"。近期研究规律（3篇）："+JSON.stringify(research)}];
    ...
    if(!apiResponse.ok){if(apiResponse.status===401) throw new Error("AI 密钥无效或已失效"); throw new Error(`AI 服务暂不可用（${apiResponse.status}）`);} const responsePayload=await apiResponse.json() as Record<string,unknown>; const draft=parseDraft(outputText(responsePayload)); const options=(Array.isArray(draft.titleOptions)?draft.titleOptions:[]).map((item)=>String(item).trim()).filter(Boolean).slice(0,3); while(options.length<3) options.push(options.length===0?draft.title:`${draft.title}｜方案${options.length+1}`); draft.titleOptions=options; draft.title=options[0]; draft.mode="AI 多图分析"; draft.coverIndex=Math.max(0,Number(draft.coverIndex||0)); await db.update(projects).set({status:"drafted",draftJson:JSON.stringify(draft)}).where(and(eq(projects.id,projectId),eq(projects.ownerEmail,ownerEmail))); return Response.json({draft});
