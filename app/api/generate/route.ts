...
export async function POST(request: Request) {
  try {
    ...
    const content:Array<Record<string,unknown>>=[{type:"input_text",text:"你是个人室内设计工作室的高级内容秘书。必须依据上传的项目实景图与已知设计信息，自动生成完整的小红书创作成品，包括3个笔记标题、封面英文栏目、封面主标题、封面副标题、封面样式、正文、话题标签、图片分析重点和发布风险提示。先识别空间类型、材质、灯光、动线、功能、人物使用场景和画面情绪，再选择最适合封面的图片序号。必须针对当前空间类型改变叙事重点、标题角度、封面选图和视觉样式，不能把住宅客厅、卧室、办公、酒店、商业和展厅写成同一套模板。当前空间专属策略："+spaceDesignGuidance(project.projectType,project.category)+"必须使用最近3篇室内设计引流笔记库中提炼的标题结构、正文节奏、客户需求洞察和自然咨询引导方法进行升级。可以学习参考标题中小表情的数量与位置，但不得复制标题、原句、段落或封面版式，不得让参考内容覆盖项目图片与真实事实。标题或正文可自然使用2至4个与空间气质匹配的小表情（例如✨🌿💜☕），标题最多1个，禁止连续堆叠，话题标签中不要放表情。只借鉴抽象规律，不虚构地点、面积、客户身份、材料品牌或完工事实。生成3个差异明确、可选择的小红书笔记标题，分别侧重空间情绪、设计亮点、实际使用价值；每个标题不超过20个汉字，避免标题党。正文必须至少写出两项与当前空间类型直接相关、且能从图片或项目资料验证的设计细节，并在结尾用自然问题邀请读者交流空间需求，不加入虚假优惠或夸张承诺。coverEyebrow 必须是简洁的大写英文，格式为 ORIGINAL DESIGN · 加空间类别，例如 INTERIOR、WORKPLACE、HOSPITALITY、RETAIL 或 EXHIBITION，最多44个字符。titleOptions 必须正好包含 3 个标题，title 必须等于 titleOptions 的第一项。根据项目图片、空间策略与引流笔记库方法推荐 coverStyle：fontFamily 只能是 serif/sans/kai；titleColor、subtitleColor、overlayColor、patternColor 使用6位十六进制颜色；overlayOpacity 为0-90；pattern 只能是 none/frame/grid/dots/corners/polka/textile/gradient/blue-white-dots；titleSize 为52-120；align 只能是 left/center；position 只能是 top/middle/bottom。只返回 JSON，字段为 title, titleOptions, coverEyebrow, coverTitle, coverSubtitle, coverStyle, body, tags, highlights, riskNotes, coverIndex。项目事实："+JSON.stringify(project)+"。近期室内设计引流笔记（只学习标题与正文结构）："+JSON.stringify(research)}];
    ...
    if(!apiResponse.ok){if(apiResponse.status===401) throw new Error("AI 密钥无效或已失效"); throw new Error(`AI 服务暂不可用（${apiResponse.status}）`);} const responsePayload=await apiResponse.json() as Record<string,unknown>; const draft=parseDraft(outputText(responsePayload)); const options=(Array.isArray(draft.titleOptions)?draft.titleOptions:[]).map((item)=>String(item).trim()).filter(Boolean).slice(0,3); while(options.length<3) options.push(options.length===0?draft.title:`${draft.title}｜方案${options.length+1}`); draft.titleOptions=options; draft.title=options[0]; draft.coverEyebrow=String(draft.coverEyebrow||defaultCoverEyebrow(project.projectType,project.category)).toUpperCase().slice(0,44); draft.mode="AI 多图分析 · 引流笔记库增强"; draft.coverIndex=Math.max(0,Number(draft.coverIndex||0)); await db.update(projects).set({status:"drafted",draftJson:JSON.stringify(draft)}).where(and(eq(projects.id,projectId),eq(projects.ownerEmail,ownerEmail))); return Response.json({draft});
