...
export function StudioSecretary() {
  ...
  const researchView = <section className="dashboard-card">
    <div className="section-heading"><div><span>DAILY CONTENT INTELLIGENCE</span><h2>每日 3 篇小红书高热参考解析</h2><p>每张参考卡片都可点击查看小红书原笔记；秘书提炼后的结构、封面层级与受众洞察会自动用于后续项目生成升级。</p></div><button className="small-action" disabled={researching} onClick={() => void runResearch(true)}>{researching ? "正在小红书研究…" : "刷新今日 3 篇"}</button></div>
    ...
    <div className="research-grid">
      {references.map((reference, index) => <a className="research-card" href={reference.sourceUrl} target="_blank" rel="noreferrer" key={reference.id}>
        ...
        <div className="source-row"><span>{reference.metricsNote}</span><strong>点击整张卡片查看小红书原笔记 ↗</strong></div>
      </a>)}
