...
版本 2.1 将浏览器右键收藏设为研究主流程：用户在当前登录的小红书原笔记页右键选择“收藏到 MJ 引流笔记库”，再回平台同步解析；本机 Playwright 仅作为可选批量工具。平台按可见互动数据排序并保存抽象的文案、封面与受众规律，生成新项目时读取这些规律，但禁止复制原句、标题或封面版式。

## 多模型视觉文案生成

生成接口会先逐张分析项目实景图，再输出封面选图、封面主副标题、5 个笔记标题、150 字以内正文、Emoji、话题标签和三种差异化方案。`QWEN_MODEL` 支持用逗号配置多个千问模型；还可通过 `XHS_AI_BASE_URL`、`XHS_AI_API_KEY` 和 `XHS_AI_MODELS` 接入任意 OpenAI-compatible 视觉模型。模型会按配置顺序重试和切换，全部不可用或返回 429 时自动使用免额度的项目规则引擎，生成按钮不会因模型余额中断。

模型轮换、结构化提示与错误兜底的设计参考了 MIT 许可的 `EBOLABOY/xhs-ai-writer`，完整说明见 `THIRD_PARTY_NOTICES.md`。
